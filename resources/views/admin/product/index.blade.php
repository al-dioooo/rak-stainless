...
@section('content')
    <div class="container grid px-6 mx-auto">
        ...
        <div class="inline-flex mt-2 mb-4 space-x-2">
            <span
                class="inline-flex px-2 text-xs font-semibold leading-5 rounded-full text-accent-1-800 bg-accent-1-100">Best</span>
            <span
                class="inline-flex px-2 text-xs font-semibold leading-5 rounded-full text-primary-800 bg-primary-100">Featured</span>
            <span
                class="inline-flex px-2 text-xs font-semibold leading-5 rounded-full text-accent-3-800 bg-accent-3-100">Promo</span>
        </div>
        <div class="container mx-auto" x-data="loadItems()">
            ...
            @else
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 w-10 h-10">
                                                    <img class="object-cover w-10 h-10 rounded-full" :src="`${item.pict}`"
                                                        alt="">
                                                </div>
                                                <div class="ml-4">
                                                    <div class="text-sm font-medium text-gray-900" x-text="item.name">
                                                    </div>
                                                    <span x-show="item.best"
                                                        class="inline-flex w-4 h-4 rounded-full bg-accent-1-100">
                                                    </span>
                                                    <span x-show="item.feature"
                                                        class="inline-flex w-4 h-4 rounded-full bg-primary-100">
                                                    </span>
                                                    <span x-show="item.sale"
                                                        class="inline-flex w-4 h-4 rounded-full bg-accent-3-100">
                                                    </span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900" x-text="item.price"></div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 capitalize" x-text="item.discount"></div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 capitalize" x-text="item.weight"></div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 capitalize" x-text="item.stock"></div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="flex items-center w-full py-6 space-x-4 md:w-1/2" x-show="pageCount() > 1">
                    <!--First Button-->
    ...
    <script>
        var sourceData = [
            @foreach ($product as $row)
                {
                id: '{{ $row->id }}',
                edit: '{{ route('admin.product.edit', ['id' => $row->id]) }}',
                name: '{!! $row->name !!}',
                slug: '{{ $row->slug }}',
                ...
                discount: '{{ $row->discount > 0 ? 'Rp ' . number_format($row->discount) : '-' }}',
                pict: '{{ asset('image/products') . '/' . $row->pict }}',
                },
            @endforeach
        ];
                ...
                get filteredItems() {
                    ...
                    this.total = this.myForData.filter((item) => {
                        return item.name.toLowerCase()
                            ...
                            .includes(this.search.toLowerCase()) ||
                            item.discount.toLowerCase()
                            .includes(this.search.toLowerCase()) ||
                            item.stock.toLowerCase()
                            .includes(this.search.toLowerCase()) ||
