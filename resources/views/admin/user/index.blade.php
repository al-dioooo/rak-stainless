...
@section('content')
    <div class="container grid px-6 mx-auto">
        ...
        <div class="container mx-auto" x-data="loadUsers()">
                ...
                <div class="flex">
                    <div class="relative w-full max-w-sm focus-within:text-primary-500">
                        ...
                        <input x-ref="searchField" x-model="search" x-on:click="viewPage(0)"
                            ...
                            type="text" placeholder="Search users" aria-label="Search">
                    </div>
                </div>
            </div>
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 w-10 h-10">
                                                <img class="w-10 h-10 rounded-full" :src="`${item.profile_image}`" alt="">
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900"
                                            x-text="item.address"></div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 capitalize" x-text="item.role"></div>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
    ...
    <script>
        var sourceData = [
            @foreach ($user as $row)
                ...
                name: '{{ $row->name }}',
                email: '{{ $row->email }}',
                address: '{{ isset($row->address->address) ? $row->address->address . ', ' . $row->address->postal_code : 'Not defined' }}',
                role: '{{ $row->role }}',
                profile_image: "https://ui-avatars.com/api/?name={{ $row->name }}&size=200&background=EBF4FF&color=7F9CF5",
                },
            @endforeach
        ];
